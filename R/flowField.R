flowField <-
function(deriv, x.lim, y.lim, parameters = NULL, points = 11, system = "two.dim", colour = "gray", arrow.type = "equal", arrow.head = 0.05, frac = 1, add = TRUE, ...){
  ...
  if ((arrow.type != "proportional") & (arrow.type != "equal")){
    stop(paste("arrow.type must either be set to proportional or equal"))
  }
  if (arrow.head <= 0){
    stop(paste("arrow.head is less than or equal to zero"))
  }
  if (frac <= 0){
    stop(paste("frac is less than or equal to zero"))
  }
  ...
  if (add == FALSE){
    plot(1, xlim = c(x.lim[1] - x.max.length, x.lim[2] + x.max.length), ylim = c(y.lim[1] - y.max.length, y.lim[2] + y.max.length), type = "n", ...)
  }
  if (system == "two.dim"){
    for (i in 1:length(x)){
      for (j in 1:length(y)){
    ...
    coefficient <- frac*min(x.max.length, y.max.length)/(2*sqrt(2)*max(sqrt(2*(abs.dy.non/abs.dx.non)/((abs.dy.non/abs.dx.non) + (abs.dx.non/abs.dy.non))), sqrt(2*(abs.dx.non/abs.dy.non)/((abs.dy.non/abs.dx.non) + (abs.dx.non/abs.dy.non)))))
                                                                   #      (abs.dx.non^2)/((abs.dy.non^2) + (abs.dx.non^2))), sqrt(2*(abs.dy.non^2)/((abs.dx.non^2) + (abs.dy.non^2)))))
    #coefficient <- frac*min(x.max.length, y.max.length)/(2*sqrt(2)*max(sqrt(2*(abs.dx.non^2)/((abs.dy.non^2) + (abs.dx.non^2))), sqrt(2*(abs.dy.non^2)/((abs.dx.non^2) + (abs.dy.non^2)))))
    for (i in 1:length(x)){
      for(j in 1:length(y)){
        if ((dx[i, j] != 0) | (dy[i, j] != 0)){
          if ((dx[i, j] != 0) & (dy[i, j] != 0)){
            factor  <- sqrt(2/((abs.dy[i, j]/abs.dx[i, j]) + (abs.dx[i, j]/abs.dy[i, j])))
          ...
          arrows(x[i] - x.shift, y[j] - y.shift, x[i] + x.shift, y[j] + y.shift, length = arrow.head, col = colour, ...)
        }
      }
    }
  }
  if (system == "one.dim"){
    ...
    coefficient <- frac*min(x.max.length, y.max.length)/(2*sqrt(2)*max(sqrt(2*abs.dy.non/(abs.dy.non + (1/abs.dy.non))), sqrt(2*(1/abs.dy.non)/(abs.dy.non + (1/abs.dy.non)))))
    for (i in 1:length(x)){
      for (j in 1:length(y)){
        if (dy[i, j] != 0){
